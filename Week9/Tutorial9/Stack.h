...
private:
    ...
    void resize(size_t aNewSize) {
        ...
        T* fNew = new T[aNewSize];
        for (size_t i = 0; i < fStackPointer; i++) {
            fNew[i] = std::move(fElements[i]);
        }
    ...
    void ensure_capacity() {
        ...
        if (fStackPointer == fCurrentSize) { resize(fCurrentSize * 2); }
    }
    void adjust_capacity() { 
        if (fStackPointer <= fCurrentSize / 4) { resize(fCurrentSize / 2); }
    }
